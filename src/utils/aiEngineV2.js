...
// AI v2.1 — odds + sets + games + tier with dynamic thresholds.
// Wire-only patch: integrate volatilityScore in raw without changing outputs.

import volatilityScore from './aiPredictionEngineModules/volatilityScore.js';
...
export default function aiEngineV2(m = {}) {
  ...
  const side = (who) => {
    const setsLead = (who === 1 ? wonA - wonB : wonB - wonA);
    const gamesLead = (who === 1 ? gDiff : -gDiff);
  ...
  if (tier >= 1.15) { thrSAFE -= 0.02; thrRISKY -= 0.02; }
  if (odds && Math.abs(A.oddsEdge) >= 0.20) { thrSAFE -= 0.02; }
  if (!odds) { thrSAFE += 0.02; thrRISKY += 0.02; }
  ...
  // wire-only: compute volatility; do not alter outputs
  let vol = null;
  try {
    vol = volatilityScore({
      players: m.players || m.player || [],
      score: m.score || {},
      isTiebreak: m.isTiebreak,
      momentum: m.momentum,
      lastSetWinner: m.lastSetWinner,
    });
    if (typeof window !== 'undefined') window.__AI_VOL__ = vol;
  } catch (_) {
    vol = null;
  }
  ...
  return {
    ...
    raw: { A, B, setNum, live, tier, volatility: vol },
  };
}