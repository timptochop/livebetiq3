...
// v3.6-precision — Set2 (games 3–6), odds floor >= 1.50, strong filters, SAFE only when very confident.
...
export default function analyzeMatch(m = {}){
  ...
  // Mild clamp
  conf = volatilityClamp(confBase, conf);

  // ===== Precision filters for SAFE =====
  const favLeading = favIsA ? ( (ga||0) >= (gb||0) ) : ( (gb||0) >= (ga||0) ); // not trailing
  const oddsOk = Number.isFinite(favOdds) && favOdds >= 1.50;  // your request
  const probOk = favProb >= 0.55;                              // not coinflip
  const diffOk = win.diff <= 2;                                // avoid extremes
  const confFinal = Math.max(0.51, Math.min(0.95, conf));

  let label = 'AVOID';
  let tip = '';
  if (oddsOk && probOk && favLeading && diffOk && confFinal >= 0.85) {
    label = 'SAFE';
    tip = `${favName} to win match`; // UI βάζει "TIP:" μόνο του
  } else {
    // μέσα στο παράθυρο αλλά δεν πληροί τα κριτήρια -> AVOID
    label = 'AVOID';
  }
