...
function breakDeltaForA(serveA, serveB, vA, vB){
  // Αν δεν ξέρουμε πόντους, ουδέτερο
  if (vA==null || vB==null) return 0;
  // Απλοί κανόνες break:
  //   - Όταν σερβίρει ο Α: break point υπέρ B σε (B=40 & A<=30) ή (B=AD & A=40)
  //   - Όταν σερβίρει ο Β: break point υπέρ A σε (A=40 & B<=30) ή (A=AD & B=40)
  ...
  // Διπλό break point (0-40) -> πιο έντονο σήμα
...
function pickOdds(pickName, odds){
  ...
  for (const k of candidates) {
    const v = odds[k];
    if (typeof v === 'number') return v; // χωρίς ταύτιση ονόματος – fallback
  }
  return null;
}
