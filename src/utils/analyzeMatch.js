...
// v3.6-soft — λίγο πιο "γενναιόδωρο": conf>=0.83, favProb>=0.54, set2Diff<=3, clamp up +0.05

const T = {
  SAFE_CONF: 0.83,     // πριν 0.85
  MIN_ODDS: 1.50,      // μένει όπως ζητήθηκε
  MIN_PROB: 0.54,      // πριν 0.55
  MAX_SET2_DIFF: 3,    // πριν 2
  CLAMP_UP: 0.05,      // πριν 0.04
  CLAMP_DOWN: -0.05
};
...
export default function analyzeMatch(m = {}){
  ...
  let label = 'AVOID';
  let tip = '';
  if (oddsOk && probOk && favLeading && diffOk && confFinal >= T.SAFE_CONF) {
    label = 'SAFE';
    tip = `${favName} to win match`; // το UI προσθέτει "TIP:" μόνο του
  }
