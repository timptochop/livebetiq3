...
// v5.0-phase1-wired + GAS-aware weights (safe merge)
import { getLbqConfig } from './lbqConfigStore';
...
// unified adaptive weights
function getAdaptiveWeights() {
  // 1) try Google Apps Script payload (lbq-config)
  try {
    const cfg = typeof getLbqConfig === 'function' ? getLbqConfig() : null;
    if (cfg && typeof cfg === 'object' && cfg.ok) {
      // sheet returns {ev, confidence, momentum, drift, surface, form, ...}
      return {
        ev: Number(cfg.ev ?? DEFAULT_WEIGHTS.ev),
        confidence: Number(cfg.confidence ?? DEFAULT_WEIGHTS.confidence),
        momentum: Number(cfg.momentum ?? DEFAULT_WEIGHTS.momentum),
        drift: Number(cfg.drift ?? DEFAULT_WEIGHTS.drift),
        surface: Number(cfg.surface ?? DEFAULT_WEIGHTS.surface),
        form: Number(cfg.form ?? DEFAULT_WEIGHTS.form),
      };
    }
  } catch (e) {
    // silent – we don't want to break render
  }

  // 2) try window.__LBQ_WEIGHTS__ (your existing runtime override)
  if (
...
export default function analyzeMatch(m = {}) {
  ...
  const confBase = 0.5 + (favProb - 0.5) * 1.2 + catBonus + liveBonus;
  let conf = confBase;
  ...
  const riskyAll =
    riskyLeadOk && minOddsOk && riskyProbOk && riskyDiffOk && confFinal >= T.RISKY_MIN_CONF;
  ...
  return {
    ...
    features: {
      ...
      set2Diff: win.diff,
      weightsSource:
        typeof window !== 'undefined' && window.__LBQ_WEIGHTS__ ? 'adaptive' : 'default-or-gas',
    },
  };
}