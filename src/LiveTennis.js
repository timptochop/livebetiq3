    ...
    <div style={{ background:'#121212', minHeight:'100vh' }}>
      ...
      {status === 'error' && (
        <div style={{ color:'#ff6b6b', textAlign:'center', paddingTop:24 }}>
          Failed to fetch ({errorMsg}). {API_BASE.startsWith('http') ? `API: ${API_BASE}` : 'Using /api'}
        </div>
      )}
        ...
        {filtered.map((m) => (
          <div key={m.id} className="prediction-card">
            <div className="top-row">
              ...
              <span className="label" style={{ backgroundColor: labelColor(m.aiLabel) }}>{m.aiLabel}</span>
            </div>
