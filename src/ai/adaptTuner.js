...
function getCutoffs() {
  try { const raw = localStorage.getItem('LBQ_CUTOFFS'); return raw ? JSON.parse(raw) : null; }
  catch { return null; }
}
function setCutoffs(c) { try { localStorage.setItem('LBQ_CUTOFFS', JSON.stringify(c)); } catch {} }

function getState() {
  try { const raw = localStorage.getItem('LBQ_ADAPT_STATE'); return raw ? JSON.parse(raw) : { noSafeStreak:0, lastStepAt:0 }; }
...
function clampCutoffs(c) {
  return {
    minEV: Math.max(FLOORS.minEV, c.minEV),
    safeConf: Math.max(FLOORS.safeConf, c.safeConf),
    riskyConf: Math.max(FLOORS.riskyConf, c.riskyConf),
  };
}

export function getCurrentCutoffs() {
  return getCutoffs() || { ...DEFAULTS };
}
...
export function ingestBatch(list) {
  try {
    ...
    const counts = arr.reduce((a, m) => {
      const L = String(m?.ai?.label || '').toUpperCase();
      if (L) a[L] = (a[L] || 0) + 1;
      return a;
    }, {});
    const safe = counts.SAFE || 0;
    const avoid = counts.AVOID || 0;

    const st = getState();
    const now = Date.now();

    if (safe === 0 && avoid >= NEED_AVOID_COUNT) st.noSafeStreak += 1;
    else st.noSafeStreak = 0;

    if (st.noSafeStreak >= NEED_STREAK && now - st.lastStepAt > COOLDOWN_MS) {
      const cur = getCutoffs() || { ...DEFAULTS };
      const next = clampCutoffs({
        minEV: Math.max(FLOORS.minEV, cur.minEV - STEPS.minEV),
        safeConf: Math.max(FLOORS.safeConf, cur.safeConf - STEPS.conf),
        riskyConf: Math.max(FLOORS.riskyConf, cur.riskyConf - STEPS.conf),
      });
      setCutoffs(next);
      st.lastStepAt = now;
    }
