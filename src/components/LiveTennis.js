...
export default function LiveTennis({ onLiveCount = () => {} }) {
  ...
  const normalized = useMemo(() => rows.map((m) => {
    ...
    return {
      ...
      name1, name2, date, time, dt, isLive, status,
  ...
  const labelPriority = { SAFE: 1, RISKY: 2, AVOID: 3, SOON: 4 };
  ...
  const badgeColors = { SAFE: '#1fdd73', RISKY: '#ff9900', AVOID: '#ff2e2e', SOON: '#9370DB', DEFAULT: '#5a5f68' };

  const setBadge = (m) => {
    const baseLabel = String(m.displayLabel || '').toUpperCase();
    let bg = badgeColors[m.label] || badgeColors.DEFAULT;
    if (baseLabel.startsWith('STARTS IN')) bg = '#5a5f68';
    return (
      <div title={m.reason || ''} style={{
        background: bg, color: '#ffffff', borderRadius: 16, padding: '6px 12px',
        fontWeight: 900, fontSize: 13, boxShadow: '0 8px 18px rgba(0,0,0,0.28)', minWidth: 64, textAlign: 'center'
      }}>
        {baseLabel}
      </div>
    );
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                <div>
                  ...
                  <div style={detailsStyle}>{m.date} {m.time} • {m.categoryName}</div>
                  {['SAFE', 'RISKY'].includes(m.label) && m.pick && (<div style={tipStyle}>TIP: {m.pick}</div>)}
                </div>
              </div>
              {setBadge(m)}
            </div>
