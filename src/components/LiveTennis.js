...
export default function LiveTennis({ onLiveCount = () => {} }) {
  ...
  // φιλτράρουμε (κρύβουμε finished) & ταξινομούμε
  const list = useMemo(() => {
    const keep = normalized.filter((m) => !isFinishedLike(m.status));

    // order: live (set desc) -> upcoming (by time)
  ...
  // badge set
  const setBadge = (m) => {
    let label = 'SET ' + (m.setNum || 1);
    if (isUpcoming(m.status)) label = 'SET 1';
    const bg = isUpcoming(m.status) ? '#5f4abf' : '#6a3bd8';
    return (
      <div
        style={{
          background: bg,
          ...
          fontWeight: 900,
          fontSize: 14,
          boxShadow: '0 6px 18px rgba(106,59,216,0.35)',
        }}
      >
        {label}
      </div>
  ...
  // spacer ώστε να ΜΗ φαίνονται από πίσω (ίδιο ύψος με TopBar)
  const TOP_SPACER = 76;

  return (
    <div style={{ background: '#0a0c0e', minHeight: '100vh' }}>
      <div style={{ height: TOP_SPACER }} /> {/* spacer κάτω από το sticky TopBar */}

      <div style={{ maxWidth: 1100, margin: '12px auto 40px', padding: '0 14px' }}>
        {loading && list.length === 0 ? null : null}
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                <div>
                  <div style={titleStyle}>
                    {m.name1} <span style={{ color: '#96a5b4', fontWeight: 600 }}>vs</span>{' '}
                    {m.name2}
                  </div>
