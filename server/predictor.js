...
function getPlayers(match) {
  ...
  const wrap = (o) => ({
    name: o.name || o['@name'] || '',
    s: [n(o.s1 || o['@s1']), n(o.s2 || o['@s2']), n(o.s3 || o['@s3']), n(o.s4 || o['@s4']), n(o.s5 || o['@s5'])]
  });
...
function baselineFromOdds(m) {
  ...
  if (typeof o.imp1 === 'number' && typeof o.imp2 === 'number') {
    ...
    if (s > 0) return { pA: o.imp1 / s, pB: o.imp2 / s, source: o.source || 'odds' };
  }
  if (typeof o.p1 === 'number' && typeof o.p2 === 'number') {
    const s = o.p1 + o.p2;
    if (s > 0) return { pA: o.p1 / s, pB: o.p2 / s, source: o.source || 'odds' };
  }
  return { pA: 0.5, pB: 0.5, source: 'fallback' };
}

function clamp01(x) { return Math.min(0.999, Math.max(0.001, x)); }
...
function updateWithScore(pA0, pB0, wa, wb, ga, gb, setNum) {
  ...
  const s = pA + pB;
  if (s > 0) { pA /= s; pB /= s; } else { pA = 0.5; pB = 0.5; }
  return { pA, pB };
}
...
function buildPrediction(m) {
  ...
  if (status === 'not started' || setNum < 3) {
    return { label: 'PENDING', pick: null, confidence: 0, source: 'fallback', detail: `set${setNum}_pending` };
  }
  ...
  return {
    ...
    probs: { A: Math.round(pA * 100), B: Math.round(pB * 100) },
...
function predictMatches(matches) {
  ...
  return arr.map((m) => ({ ...m, prediction: buildPrediction(m) }));
}
